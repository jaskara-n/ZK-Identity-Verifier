...
```bash
curl -X POST http://localhost:5001/api/verifier/clients/register \
...
```bash
curl -X POST http://localhost:5001/api/verifier/auth/token \
...
## 3) Create challenge
```bash
curl -X POST http://localhost:5001/api/verifier/challenges \
  -H 'content-type: application/json' \
...
```bash
curl http://localhost:5001/api/verifier/challenges/<CHALLENGE_ID>/public
```
...
```bash
curl -X POST http://localhost:5001/api/verifier/challenges/<CHALLENGE_ID>/submit \
  -H 'content-type: application/json' \
...
```bash
curl http://localhost:5001/api/verifier/challenges/<CHALLENGE_ID> \
  -H 'authorization: Bearer <ACCESS_TOKEN>'
```

## 8) Verifier lists issued credentials
```bash
curl http://localhost:5001/api/verifier/credentials \
  -H 'authorization: Bearer <ACCESS_TOKEN>'
```

## 9) Verifier revokes credential
```bash
curl -X POST http://localhost:5001/api/verifier/credentials/<CREDENTIAL_ID>/revoke \
  -H 'content-type: application/json' \
  -H 'authorization: Bearer <ACCESS_TOKEN>' \
